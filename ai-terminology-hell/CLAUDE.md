...
non-technical people, and many technical ones, feeling overwhelmed. This folder exists to make the most important
...
- **Later:** this folder grows into a broader explainer resource. Possible additions include a glossary,
...
26. **Bridge question**: "how do we get from predicting the next token to an agent that operates Outlook?", light statement slide, question only (`scripts/slide_bridge_question.py`)
...
- **Paths:** this folder is `ai-terminology-hell/` inside the user's `ai-classroom` repository. Run commands from
  here; Python comes from the repo's shared `.venv` one folder up (`../.venv/bin/python`). Deck paths passed to
  scripts are relative to this folder. The scripts find the deck and `assets/` on their own, so they also work
...
- The `.pptx` is the source of truth. Edit it programmatically (python-pptx, or direct OOXML edits);
  `python-pptx` 1.0.2 is installed in the shared `../.venv`; run scripts with `../.venv/bin/python`.
...
- Don't install tools globally (no Homebrew / LibreOffice / poppler). Python dependencies go into the shared `../.venv` only (listed in the repo's `requirements.txt`).
